...
| embed          | テキスト/画像埋め込み生成（既定: HuggingFace CLIP）  | `ragserver/embed/`、`embed_server/`                | `CLIPEmbeddingsManager` が `embed_server/clip_embed_server.py` に接続      |
| rerank         | 検索結果のリランク（既定: HuggingFace BGE reranker） | `ragserver/rerank/`、`rerank_server/`              | `HFRerankManager` が `rerank_server/flagembedding_rerank_server.py` に接続 |
...
- embed: `ragserver/embed/` に埋め込みのインタフェース。既定は `embed_server/clip_embed_server.py`（HuggingFace CLIP）を叩く構成で、OpenAI/Cohere などへの切り替えも環境変数で対応。
- rerank: `ragserver/rerank/` にリランク機構。既定は HuggingFace BGE (`rerank_server/flagembedding_rerank_server.py`) を使用。
...
| `EMBED_PROVIDER`                | 埋め込みプロバイダ選択                  | `clip` / `openai` / `cohere` |
...
| `CLIP_EMBED_MODEL_TEXT`         | HuggingFace CLIP 埋め込みテキストモデル | 文字列                       |
| `CLIP_EMBED_MODEL_IMAGE`        | HuggingFace CLIP 埋め込み画像モデル     | 文字列                       |
| `CLIP_EMBED_BASE_URL`           | HuggingFace CLIP 埋め込み API の URL    | URL                          |
...
| `FLAGEMBEDDING_RERANK_MODEL`    | HuggingFace リランカーモデル名          | 文字列                       |
| `FLAGEMBEDDING_RERANK_BASE_URL` | HuggingFace リランカー API の URL       | URL                          |
...
`ragserver/core/exts.Exts` で許可された拡張子：
...
- 形式: `[embed provider]__[model name]__[text|image]`
  - 例: `clip__openai/clip-vit-base-patch32__text`
- 埋め込みやストア構成を切り替えると異なる空間キーが生成され、既存データとは別コレクションとして扱われます。モデルを切り替えた場合は再インジェストが必要です。
- マルチモーダル構成ではテキスト用 `space_key` と画像用 `space_key_image` の 2 種が存在し、それぞれ別コレクションに格納されます。
